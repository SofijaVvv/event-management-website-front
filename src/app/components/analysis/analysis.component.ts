...
export class AnalysisComponent implements OnInit, OnDestroy {
    ...
    this.langChangeSubscription = this.translate.onLangChange.subscribe(
      (event: { lang: string }) => {
        this.chart!.options!.plugins!.title!.text =
          this.translate.instant('event.diagram');
  ...
  sumAnalysisData() {
    const totalRevenue = this.analysisData.totalRevenues.reduce(
      (a: number, b: number) => a + b,
      0,
    );
    const totalCost = this.analysisData.totalCosts.reduce(
      (a: number, b: number) => a + b,
      0,
    );
  ...
  loadTabData(startDate: string, endDate: string) {
    this.apiCalls
      .getEventListById(0, startDate, endDate)
      .subscribe((data: EventDetails[]) => {
        this.tabEventData = data;
      });
    this.apiCalls
      .getEventCosts(0, startDate, endDate)
      .subscribe((data: EventCostsDetails[]) => {
        this.tabEventCostData = data;
      });
    this.apiCalls
      .getOtherCosts(startDate, endDate)
      .subscribe((data: CostsDetails[]) => {
        this.tabCostData = data;
      });
    this.apiCalls
      .getAnalisysRevenueData(startDate, endDate)
      .subscribe((data: RevenuesAnalisysDetails[]) => {
  ...
  sumPerParticipans() {
    this.finalAnalisysData.revenuePerPerticipant =
      this.analisysDataTotals.total_revenue /
      this.analisysDataTotals.number_of_participants;
    this.finalAnalisysData.costPerPerticipant =
      this.analisysDataTotals.total_events_cost /
      this.analisysDataTotals.number_of_participants;
    this.finalAnalisysData.revenuePerEvent =
      this.analisysDataTotals.total_revenue /
      this.analisysDataTotals.number_of_events;
    this.finalAnalisysData.costPerEvent =
      this.analisysDataTotals.total_events_cost /
      this.analisysDataTotals.number_of_events;
    this.finalAnalisysData.participantsPerEvent = Math.round(
      this.analisysDataTotals.number_of_participants /
        this.analisysDataTotals.number_of_events,
    );
  }
