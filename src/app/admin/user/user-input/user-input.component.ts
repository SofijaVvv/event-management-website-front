  ...
  loadRoleList() {
    this.serviceCalls.roleList().subscribe((data: IRoles[]) => {
      ...
      this.userForm.patchValue({
        roles_combo: this.getRoleDetails(this.userForInput.roles_id || 0),
      });
      // }
      console.log(data);
    });
  ...
  resetPassword() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.serviceCalls
          .resetPassword(this.userForInput.email)
  ...
  onSubmit() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (this.userForInput.id == 0) {
          this.serviceCalls
            .createUser(JSON.stringify(user_data))
            .subscribe((odgovor: any) => {
              console.log(odgovor, 'odgovor');
              void this.spinner.hide();
              if (odgovor.error === true) {
          ...
          this.serviceCalls
            .editUser(JSON.stringify(user_data))
            .subscribe((odgovor: any) => {
              console.log(odgovor, 'odgovor');
              void this.spinner.hide();
              if (odgovor.error === true) {
